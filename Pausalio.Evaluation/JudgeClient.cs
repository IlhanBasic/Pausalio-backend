...
namespace Pausalio.Evaluation
{
    public class JudgeClient
    {
        ...
        public async Task EvaluateResultAsync(EvalResult result)
        {
            if (result.Ignored)
            {
                _logger.LogInformation("Skipping judge call for ignored/failed question ID {Id}", result.QuestionId);
                return;
            }

            _logger.LogInformation("Calling judge for question ID {Id}...", result.QuestionId);
...
clarity = jasnoća i razumljivost odgovora na srpskom jeziku";
            ...
            try
            {
                ...
                var request = new HttpRequestMessage(HttpMethod.Post, _settings.OpenRouterApiUrl)
                {
                ...
                {
                    request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", _settings.OpenRouterApiKey);
                }
                ...
                var (reasoning, accuracy, completeness, clarity) = ParseJudgeResponse(messageContent);
                ...
                _logger.LogInformation("Judge evaluation completed for question ID {Id}. Accuracy: {Acc}/5, Completeness: {Comp}/2, Clarity: {Clar}/3",
                    result.QuestionId, accuracy, completeness, clarity);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to run judge evaluation for question ID {Id}", result.QuestionId);
                result.JudgeReasoning = $"Greška prilikom pozivanja judge modela: {ex.Message}";
        ...
        private (string reasoning, int accuracy, int completeness, int clarity) ParseJudgeResponse(string rawContent)
        {
            ...
            try
            {
                ...
                string reasoning = root.TryGetProperty("reasoning", out var reasoningProp)
                    ? reasoningProp.GetString() ?? string.Empty
                    : string.Empty;

                int accuracy = root.TryGetProperty("accuracy", out var accProp)
                    ? (accProp.ValueKind == JsonValueKind.Number ? accProp.GetInt32() : (int.TryParse(accProp.GetString(), out var tempAcc) ? tempAcc : 0))
                    : 0;

                int completeness = root.TryGetProperty("completeness", out var compProp)
                    ? (compProp.ValueKind == JsonValueKind.Number ? compProp.GetInt32() : (int.TryParse(compProp.GetString(), out var tempComp) ? tempComp : 0))
                    : 0;

                int clarity = root.TryGetProperty("clarity", out var clarProp)
                    ? (clarProp.ValueKind == JsonValueKind.Number ? clarProp.GetInt32() : (int.TryParse(clarProp.GetString(), out var tempClar) ? tempClar : 0))
                    : 0;
            ...
            {
                _logger.LogWarning(ex, "Failed to parse judge JSON. Content was: \"{Content}\"", rawContent);
                return ($"Neuspešno parsiranje odgovora sudije: {ex.Message}. Originalni odgovor: {rawContent}", 0, 0, 0);
            }
        }
